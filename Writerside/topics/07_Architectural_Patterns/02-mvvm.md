...
This awareness ensures LiveData 
...
- Follow these steps to work with LiveData objects:
  ...
  - Create an Observer object that defines the `onChanged()` method, 
    which controls what happens **when the LiveData object's held data changes**. 
...
> You usually attach the Observer object in a UI controller, 
> such as an activity or fragment.

> When you update the value stored in the LiveData object, 
> it triggers all registered observers as long as the attached 
> LifecycleOwner is in the active state.

### Using LiveData objects

A LiveData object is usually 
**stored within a ViewModel object** and is accessed via a getter method.
